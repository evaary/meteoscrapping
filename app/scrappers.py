...
'''BaseScrapper : scrapper qui récupère les données brutes d'une table html'''
class BaseScrapper():
    def __init__(self, config):
        ...
        '''génération de tous les couples année-mois à utiliser pour récupérer les données'''
        self._todos = (
            (year, month)
            for year in range(config["year"]["from_"], config["year"]["to_"] +1)
            for month in range(config["month"]["from_"], config["month"]["to_"] +1)
        )

        '''listes des urls qui posent problèmes'''
        self.errors = []

    def _set_url(self, year, month):
        '''_set_url : créer l'adresse url où se trouvent les données à récupérer
        ...
        try:
            url = self._base_url + f"&ano={y}&mes={self._associations[m]['num']}&day=0&hora=0&min=0&ndays={self._associations[m]['days']}"
    ...
    def _find_table_in_html(self, html_page):
    ...
    def _scrap_data(self):
        ...
        for todo in self._todos:
            ...
            try:
                ...
                print("\t error while loading html page")
                self.errors += [url]
                continue
            # (3)
            try:
            ...
            try:
                ...
                print("\t error while reworking data")
                self.errors += [url]
                continue
...
''' OgimetScrapper : scrapper pour le site ogimet'''
class OgimetScrapper(BaseScrapper):
    ...
    def _scrap_columns_names(table):
        ...
        et leurs compléments. On remplace les sauts de lignes par des _ .
        On passe en minuscules avec lower et on supprime les espaces avec strip
        On remplace les espaces par des _. On reforme l'unité de température
    ...
    @staticmethod
    def _fill_missing_values(values, n_rows, n_cols, n_filled, month):
    ...
    def _rework_data(self, values, col_names, year, month):
        ...
        n_rows = self._associations[month]["days"]
        n_filled = n_rows * n_cols
        n_values = len(values)
        
        if n_values != n_filled:
            values = self._fill_missing_values(values, n_rows, n_cols, n_filled, month)
        # (2)
...
''' WundergroundScrapper : scrapper pour le site wunderground'''
class WundergroundScrapper(BaseScrapper):
    ''' Critère de sélection qui servira à récupérer le tableau voulu'''
    _criteria = ("aria-labelledby", "History days")
    ...
    @staticmethod
    def _rework_data(values, main_names, year, month):
        ...
        for old_unit, dico in units_conversion.items():
